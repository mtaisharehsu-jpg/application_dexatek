...
static int check_can_delete_session(const http_request_t *request, const char *id_or_token, int is_numeric_id, http_response_t *response)
{
    ...
    int target_session_id = -1;
    int requesting_session_id = -1;
    
    if (count > 0) {
        if (is_numeric_id) {
            // Deleting by session ID - need to find both target session and requesting session
            long want_id = strtol(id_or_token, NULL, 10);
            target_session_id = (int)want_id;
            
            // Find the target session
            for (int i = 0; i < count; i++) {
                if (sessions[i].id == target_session_id) {
                    owner = sessions[i].username;
                    break;
                }
            }
            
            // Find the requesting session (the one with the token being used)
            // Extract X-Auth-Token from request headers
            const char *request_token = NULL;
            for (int i = 0; i < request->header_count; i++) {
                if (strncmp(request->headers[i][0], "X-Auth-Token", 12) == 0) {
                    request_token = request->headers[i][1];
                    break;
                }
            }
            ...
            if (request_token) {
                for (int i = 0; i < count; i++) {
                    if (sessions[i].token[0] != '\0' && strcmp(sessions[i].token, request_token) == 0) {
                        requesting_session_id = sessions[i].id;
                        break;
                    }
                }
            }
        } else {
            // Deleting by token - this is the same session
            for (int i = 0; i < count; i++) {
                if (sessions[i].token[0] != '\0' && strcmp(sessions[i].token, id_or_token) == 0) {
                    owner = sessions[i].username;
                    target_session_id = sessions[i].id;
                    requesting_session_id = sessions[i].id; // Same session
                    break;
                }
            }
        }
    ...
    // Non-admin: allow only if deleting own session AND the session token belongs to that session
    int allowed = 0;
    if (strcasecmp(req_username, owner) == 0) {
        if (is_numeric_id) {
            // For numeric ID deletion: must be deleting the same session that the token belongs to
            allowed = (target_session_id == requesting_session_id) ? 1 : 0;
        } else {
            // For token deletion: always allowed since it's the same session
            allowed = 1;
        }
    }
    free(sessions);
...
// On failure, populates response with 403 InsufficientPrivilege and returns FAIL.
static int check_configure_admin_privilege(const http_request_t *request, http_response_t *response)
{
    char req_username[128] = {0};
    char req_role[64] = {0};
    ...
    if (strcmp(request->method, HTTP_METHOD_GET) == 0 || strcmp(request->method, HTTP_METHOD_HEAD) == 0) {
        ...
        switch (resource_type) {
            ...
            case REDFISH_RESOURCE_ACCOUNTSERVICE:
                ...
                break;
            case REDFISH_RESOURCE_ACCOUNTSERVICE_PRIVILEGEMAP:
                handler_result = handle_account_service_privilege_map(response);
                break;
        ...
        switch (resource_type) {
            case REDFISH_RESOURCE_MANAGER_SECURITY_POLICY:
                if (check_configure_admin_privilege(request, response) != SUCCESS) return SUCCESS;
            ...
            case REDFISH_RESOURCE_CDU_OEM_KENMEC_CONFIG_WRITE:
                if (check_configure_components_privilege(request, response) != SUCCESS) return SUCCESS;
                handler_result = handle_cdu_oem_kenmec_config_write(resource_id, request, response);
                break;

            // case REDFISH_RESOURCE_CDU_OEM_LOG_FILE:
            //     handler_result = handle_cdu_oem_log_file_get(resource_id, request, response);
            //     break;
            ...
            case REDFISH_RESOURCE_CDU_OEM_IOBOARD_OTA: {
                // debug(tag, "request->path: %s", request->path);
                if (check_configure_admin_privilege(request, response) != SUCCESS) return SUCCESS;
                const char *anchor = "/Oem/Kenmec/IOBoards/";
                const char *pos = strstr(request->path, anchor);
                const char *member = NULL;
                ...
                // debug(tag, "member: %s", member);
                // debug(tag, "resource_id: %s", resource_id);
                handler_result = handle_cdu_oem_ioboard_ota(resource_id, member, request, response);
                break;
            }
            ...
            case REDFISH_RESOURCE_CDU_OEM_CONTROL_LOGICS_ACTION_WRITE: {
                ...
                handler_result = handle_cdu_oem_control_logics_action_write(resource_id, member, request, response);
                break;
            }
...
redfish_resource_type_t parse_redfish_path(const char *path, char **resource_id) {
    ...
    if (strncmp(path, "redfish/v1/AccountService", 25) == 0) {
        ...
        if (path[25] == '/' && strncmp(&path[26], "PrivilegeMap", 12) == 0 && strlen(path) == 38) {
            return REDFISH_RESOURCE_ACCOUNTSERVICE_PRIVILEGEMAP;
        }
    ...
    if (strncmp(path, "redfish/v1/ThermalEquipment", 27) == 0) {
        ...
        if (path[27] == '/') {
            ...
            // Check IOBoard OTA path EARLY: /redfish/v1/ThermalEquipment/CDUs/{cdu}/Oem/Kenmec/IOBoards/{member}/OTA
            {
                const char *base = path;
                const char *prefix = "redfish/v1/ThermalEquipment/CDUs/";
                if (strncmp(base, prefix, strlen(prefix)) == 0) {
                    ...
                    if (slash) {
                        ...
                        if (id_len > 0 && id_len < 32) {
                            ...
                            if (m == rest && t && t[strlen(tail)] == '\0' && m < t) {
                                *resource_id = cdu_id;
                                return REDFISH_RESOURCE_CDU_OEM_IOBOARD_OTA;
                            }
                        }
                    }
                }
            ...
            // Check path: /redfish/v1/ThermalEquipment/CDUs/{id}/Oem/Kenmec/LogFile
            // {
            //     const char *anchor = "/Oem/Kenmec/LogFile";
            //     char *p = strstr((char*)path + 28, anchor);
            //     if (p && p[strlen(anchor)] == '\0') {
            //         // Extract CDU ID
            //         char *cdu_path = (char*)(path + 28);
            ...
            //                     cdu_id[len] = '\0';
            //                     *resource_id = cdu_id;
            //                     return REDFISH_RESOURCE_CDU_OEM_LOG_FILE;
            //                 }
            //             }
            //         }
            //     }
...
// Local function for HTTP socket reading with timeout
static int http_socket_read(int socket_fd, char *buffer, size_t buffer_size, int32_t timeout_ms) {
    if (socket_fd < 0 || !buffer || buffer_size == 0) {
        return ERROR_INVALID_PARAM;
    }

    int32_t byte_recv_total = 0;
    int32_t byte_recv = 0;
    int so_error;
    socklen_t errlen = sizeof(so_error);
    uint64_t enter_time = time_get_current_ms();
    ...
    do {
        ...
        // Calculate remaining buffer space and adjust pointer
        void *current_buffer = (void *)((char *)buffer + byte_recv_total);
        size_t remaining_size = buffer_size - byte_recv_total;
        
        byte_recv = recv(socket_fd, current_buffer, remaining_size, 0);
        
        // Check socket error status
        getsockopt(socket_fd, SOL_SOCKET, SO_ERROR, &so_error, &errlen);
        if (so_error && so_error != EAGAIN) {
            return ERROR_NETWORK;
        }

        if (byte_recv > 0) {
            byte_recv_total += byte_recv;
        }
        else if (byte_recv < 0) {
            if (errno == ECONNRESET || errno == ECONNABORTED) {
                if (byte_recv_total == 0) {
                    byte_recv_total = ERROR_NETWORK;
                }
                break;
            }
                ...
                error(tag, "HTTP socket read error: %s", strerror(errno));
                byte_recv_total = ERROR_NETWORK;
                break;
            }
        }
    ...
    } while ((byte_recv_total < (int32_t)buffer_size) && !time_after(enter_time + timeout_ms, time_get_current_ms()));
    ...
    if (byte_recv_total >= 0 && byte_recv_total < (int32_t)buffer_size) {
        buffer[byte_recv_total] = '\0';
    }

    return byte_recv_total;
}
...
int handle_http_client_connection_direct(int client_fd) {
...
#if DEBUG_REDISH_SERVER
    debug(tag, "Handling HTTP client connection on fd %d", client_fd);
#endif

    // Read request from client (5 second timeout)
    int bytes_read = http_socket_read(client_fd, request_buffer, sizeof(request_buffer) - 1, 5000);
    if (bytes_read <= 0) {
    ...
    int is_upload = 0;
    if (strcmp(request.method, HTTP_METHOD_POST) == 0 && 
        (strstr(request.path, "/UpdateFirmwareMultipart") != NULL || 
         (strstr(request.path, "/Oem/Kenmec/IOBoards/") != NULL && strstr(request.path, "/OTA") != NULL))) {
        is_upload = 1;
    }
    ...
    if (!is_upload) {
        if (expected_len > request.content_length && expected_len < (int)sizeof(request.body)) {
            ...
            while (remaining > 0) {
                int n = http_socket_read(client_fd, request.body + offset, remaining, 5000);
                if (n <= 0) {
        ...
        // Create appropriate temp file for upload
        if (strstr(request.path, "/Oem/Kenmec/IOBoards/") != NULL && strstr(request.path, "/OTA") != NULL) {
            strcpy(request.upload_tmp_path, "/tmp/upload.bin");
        } else {
        ...
        if (request.content_length > 0) {
            ...
            // Print initial progress (only on new whole percent)
            if (expected_len > 0) {
                int received_total = request.content_length;
                int percent = (int)((long long)received_total * 100 / expected_len);
                if (percent > last_percent) {
                    last_percent = percent;
                    printf("Receiving upload: %d/%d bytes (%d%%)\n", received_total, expected_len, percent);
                }
            }
        }
        int remaining = expected_len - request.content_length;
        ...
        while (remaining > 0) {
            ...
            int n = http_socket_read(client_fd, chunk, to_read, 5000);
            if (n <= 0) {
            ...
            remaining -= n;
            // Print incremental progress (only on new whole percent)
            if (expected_len > 0) {
                int received_total = expected_len - remaining;
                int percent = (int)((long long)received_total * 100 / expected_len);
                if (percent > last_percent) {
                    last_percent = percent;
                    printf("Receiving upload: %d/%d bytes (%d%%)\n", received_total, expected_len, percent);
...
// Thread data structure to pass client file descriptor to thread
typedef struct {
    int client_fd;
} client_thread_data_t;

// Thread wrapper function for handle_client_connection (HTTPS)
void* client_connection_thread(void* arg) {
    if (!arg) {
        debug(tag, "Error: NULL argument passed to client thread");
        return NULL;
    }
    ...
    debug(tag, "Starting HTTPS client connection thread for fd %d", client_fd);
    
    // Call the actual connection handler
    int result = handle_client_connection(client_fd);
    ...
    debug(tag, "HTTPS client connection thread completed for fd %d with result %d", client_fd, result);
    
    return NULL;
}

// Thread wrapper function for HTTP client connections
void* http_client_connection_thread(void* arg) {
    if (!arg) {
        debug(tag, "Error: NULL argument passed to HTTP client thread");
        return NULL;
    }
...
#if DEBUG_REDISH_SERVER
    debug(tag, "Starting HTTP client connection thread for fd %d", client_fd);
#endif
    
    // Process the HTTP client connection directly
    // Note: handle_http_client_connection already closes the socket, so we don't close it here
#if DEBUG_REDISH_SERVER
    int result = handle_http_client_connection_direct(client_fd);
#else
    handle_http_client_connection_direct(client_fd);
#endif
...
#if DEBUG_REDISH_SERVER
    debug(tag, "HTTP client connection thread completed for fd %d with result %d", client_fd, result);
#endif
...
int handle_https_client_connection(int client_fd) {
    ...
    if (!thread_data) {
        debug(tag, "Error: Failed to allocate memory for thread data");
        close(client_fd);
        return ERROR_MEMORY;
    }
    ...
    int thread_result = pthread_create(&client_thread, NULL, client_connection_thread, thread_data);
    
    if (thread_result != 0) {
        debug(tag, "Error: Failed to create client thread, error code: %d", thread_result);
        free(thread_data);
        close(client_fd);
    ...
    debug(tag, "Successfully created thread for client connection on fd %d", client_fd);
...
int handle_http_client_connection(int client_fd) {
    ...
    client_thread_data_t* thread_data = malloc(sizeof(client_thread_data_t));
    if (!thread_data) {
        debug(tag, "Error: Failed to allocate memory for HTTP thread data");
        close(client_fd);
        return ERROR_MEMORY;
    }
    ...
    // Create thread to handle the HTTP client connection
    pthread_t client_thread;
    int thread_result = pthread_create(&client_thread, NULL, http_client_connection_thread, thread_data);
    
    if (thread_result != 0) {
        debug(tag, "Error: Failed to create HTTP client thread, error code: %d", thread_result);
        free(thread_data);
        close(client_fd);
        return ERROR_GENERAL;
    }
...
#if DEBUG_REDISH_SERVER
    debug(tag, "Successfully created thread for HTTP client connection on fd %d", client_fd);
#endif
...
int redfish_server_post_action(label_post_action_t action, const char *context) {
    ...
    switch (action) {
        ...
        case LABEL_POST_ACTION_FIRMWARE_UPDATE:
            printf("[%s] Executing firmware update for context: %s\n", __FUNCTION__, context);
            system_firmware_update();
            printf("[%s] Firmware update initiated for %s\n", __FUNCTION__, context);
            break;
