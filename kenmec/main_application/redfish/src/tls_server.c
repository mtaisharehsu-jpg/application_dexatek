...
int tls_server_init(tls_server_context_t *ctx, const char *cert_file, const char *key_file, const char *client_cert_file, int port) {
    ...
    // Set read timeout to 5000ms (5 seconds) for operations using f_recv_timeout
...
int tls_server_establish_ssl(tls_server_context_t *ctx, int client_fd, mbedtls_ssl_context *ssl, mbedtls_net_context *client_net_ctx) {
    ...
    // Set socket-level timeout to ensure reliable timeout behavior
    struct timeval timeout;
    timeout.tv_sec = 5;  // 5 seconds
    timeout.tv_usec = 0;
    
    if (setsockopt(client_fd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(struct timeval)) < 0) {
        printf("Failed to set socket receive timeout: %s\n", strerror(errno));
        return ERROR_NETWORK;
    }
    
    if (setsockopt(client_fd, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(struct timeval)) < 0) {
        printf("Failed to set socket send timeout: %s\n", strerror(errno));
        return ERROR_NETWORK;
    }
    ...
    // Set the underlying socket using proper mbedTLS BIO functions with timeout support
    // Provide mbedtls_net_recv_timeout so that reads can time out after 5 seconds
    mbedtls_ssl_set_bio(ssl, client_net_ctx, mbedtls_net_send, mbedtls_net_recv, mbedtls_net_recv_timeout);
...
int tls_server_read(mbedtls_ssl_context *ssl, char *buffer, size_t buffer_size) {
    ...
    if (bytes_read < 0) {
        ...
        if (bytes_read == MBEDTLS_ERR_SSL_TIMEOUT) {
            // Timeout occurred - this should now work with socket-level timeout
            printf("SSL read timeout after 5 seconds\n");
            return ERROR_NETWORK;
        }
