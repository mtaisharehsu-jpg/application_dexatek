...
int handle_client_connection(int client_fd) {
    ...
    if (bytes_read <= 0) {
        error(tag, "Failed to read request from client %d", bytes_read);
    ...
    if (ret != SUCCESS) {
        error(tag, "Failed to parse HTTP request %d", ret);
        tls_server_close_client(&ssl, client_fd);
        return ret;
    }
    ...
    // upload URI: /UpdateFirmwareMultipart or /Oem/Kenmec/IOBoards/x/OTA
    ...
    if (!is_upload) {
        ...
        // Stream multipart body to appropriate temp file path
        ...
        if (request.content_length > 0) {
            ...
            if (expected_len > 0) {
                ...
                if (percent > last_percent) {
                    ...
                    printf("Receiving upload (HTTPS): %d/%d bytes (%d%%)\n", received_total, expected_len, percent);
                }
            }
        }
        int remaining = expected_len - request.content_length;
