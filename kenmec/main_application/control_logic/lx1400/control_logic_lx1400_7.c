/*
 * control_logic_lx1400_7.c - LX1400 雙DC泵控制邏輯 (Control Logic 7: 2DC Pump Control)
 *
 * 【功能概述】
 * 本模組實現 LX1400 機型的雙DC泵浦控制功能(僅2台),使用雙PID協調控制(流量+壓力)。
 * 相較於AC泵,DC泵具有更精確的速度控制和更低的能耗。
 *
 * 【控制目標】
 * - 流量控制: 維持設定流量 (預設值視配置而定)
 * - 壓力控制: 維持設定壓力 (預設值視配置而定)
 * - 雙泵協調運行,負載均衡
 *
 * 【感測器配置】
 * - F2 流量: 二次側出水流量
 * - P12 壓力: 二次側進水壓力
 * - DC泵1回饋: 頻率/電流/電壓/功率/效率
 * - DC泵2回饋: 頻率/電流/電壓/功率/效率
 *
 * 【執行器控制】
 * - DC泵1: 速度設定/啟停/復歸/故障狀態
 * - DC泵2: 速度設定/啟停/復歸/故障狀態
 *
 * 【控制模式】
 * - 手動模式: 操作員控制泵浦速度和啟停
 * - 自動模式: 雙PID控制 + 雙泵協調策略
 ...
 *
 * 【雙PID參數】
 * - 流量PID: Kp=1.2, Ki=0.15, Kd=0.06
 * - 壓力PID: Kp=1.8, Ki=0.25, Kd=0.1
 *
 * 【安全保護】
 * - 電流限制: 最大電流保護
 * - 電壓限制: 過壓/欠壓保護
...
static const char* debug_tag = "lx1400_7_2dc_pump";  // 日誌標籤

#define CONFIG_REGISTER_FILE_PATH "/usrdata/register_configs_lx1400_7.json"
...
// DC泵浦控制寄存器 (只有2台泵浦)
static uint32_t DC_PUMP1_SPEED_CMD_REG = 411037;   // DC泵1轉速設定
static uint32_t DC_PUMP1_ENABLE_CMD_REG = 411101;   // DC泵1啟停控制
static uint32_t DC_PUMP1_RESET_CMD_REG = 411102;   // DC泵1異常復歸
static uint32_t DC_PUMP1_STATUS_REG = 411109;   // DC泵1過載狀態
static uint32_t DC_PUMP1_FREQ_FB_REG = 42501;    // DC泵1輸出頻率
static uint32_t DC_PUMP1_CURRENT_FB_REG = 42502;    // DC泵1電流
static uint32_t DC_PUMP1_VOLTAGE_FB_REG = 42503;    // DC泵1電壓

static uint32_t DC_PUMP2_SPEED_CMD_REG = 411039;   // DC泵2轉速設定
static uint32_t DC_PUMP2_ENABLE_CMD_REG = 411103;   // DC泵2啟停控制
static uint32_t DC_PUMP2_RESET_CMD_REG = 411104;   // DC泵2異常復歸
static uint32_t DC_PUMP2_STATUS_REG = 411110;   // DC泵2過載狀態
static uint32_t DC_PUMP2_FREQ_FB_REG = 42511;    // DC泵2輸出頻率
static uint32_t DC_PUMP2_CURRENT_FB_REG = 42512;    // DC泵2電流
static uint32_t DC_PUMP2_VOLTAGE_FB_REG = 42513;    // DC泵2電壓
...
static uint32_t PUMP1_STOP_REG = 45025;    // 泵1停用
static uint32_t PUMP2_STOP_REG = 45026;    // 泵2停用
