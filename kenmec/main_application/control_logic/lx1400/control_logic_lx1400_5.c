/*
 * control_logic_lx1400_5.c - LX1400 補水泵控制邏輯 (Control Logic 5: Water Pump Control)
 *
 * 【功能概述】
 * 本模組實現 LX1400 機型的補水泵控制功能,根據液位自動補水,維持水箱液位在安全範圍。
 * 實現手動控制與自動補水功能。
 *
 * 【控制目標】
 * - 維持水箱液位在高低液位之間
 ...
 *
 * 【感測器配置】
 * - 高液位DI (REG 411112): 0=無液位, 1=有液位
 * - 低液位DI (REG 411113): 0=無液位, 1=有液位
 * - 漏液檢測DI (REG 411114): 0=正常, 1=漏液
 * - 系統狀態 (REG 42001): bit8=液位狀態
 *
 * 【執行器控制】
 * - 補水泵DO (REG 411108): 0=停止, 1=運行
 *
 * 【控制模式】
 * - 手動模式: 操作員控制補水泵啟停
 * - 自動模式: 根據液位自動補水
 ...
 *
 * 【運行狀態機】
 * IDLE → STARTING → RUNNING → STOPPING → COMPLETED
 * 異常狀態: TIMEOUT, ERROR
 *
 * 【配置參數】
 * - 目標壓力: 2.0 bar (預設)
 * - 啟動延遲: 2000 ms
 * - 最大運行時間: 300000 ms (5分鐘)
 * - 完成延遲: 1000 ms
 * - 最大失敗次數: 3次
 *
 * 【安全保護】
...
// CDU 補水泵控制邏輯程式
// 實現手動控制與自動補水功能 (依據 cdu_water_pump_control.md，簡化版本)
// ========================================================================================
// CDU 補水泵寄存器定義 (依據 CDU 控制系統 Modbus 寄存器定義表)
// ========================================================================================

static const char* tag = "lx1400_5_water_pump";

#define CONFIG_REGISTER_FILE_PATH "/usrdata/register_configs_lx1400_5.json"
...
static uint32_t REG_WATER_PUMP_CONTROL = 411108;   // 補水泵啟停控制 (0=Stop, 1=Run)

// 液位檢測寄存器
static uint32_t REG_HIGH_LEVEL = 411112;   // CDU水箱_高液位檢 (0=無液位, 1=有液位)
static uint32_t REG_LOW_LEVEL = 411113;   // CDU水箱_低液位檢 (0=無液位, 1=有液位)
static uint32_t REG_LEAK_DETECTION = 411114;   // 漏液檢 (0=正常, 1=漏液)
...
static bool read_water_pump_status(water_pump_status_t* status) {
    ...
    debug(tag, "pump_control = %d (DO_7)(%d)", pump_control, REG_WATER_PUMP_CONTROL);
    debug(tag, "high_level = %d (DI_3)(%d)", high_level, REG_HIGH_LEVEL);
    ...
    bool success = (pump_control != 0xFFFF) && (high_level != 0xFFFF) && 
                   (low_level != 0xFFFF) && (leak_detection != 0xFFFF) && 
...
static int _register_list_init(void)
{
    ...
    _control_logic_register_list[3].name = REG_LOW_LEVEL_STR;
    _control_logic_register_list[3].address_ptr = &REG_LOW_LEVEL;
    _control_logic_register_list[3].default_address = REG_LOW_LEVEL;
