/*
 * control_logic_ls300d_4.c - LS80 泵浦控制邏輯 (Control Logic 4: Pump Control)
 *
 * 【功能概述】
 * 本模組實現 CDU 系統的AC泵浦控制功能,通過雙 PID 演算法(流量/壓差)維持系統穩定運行,
 * 支援手動/自動模式切換,並提供完整的泵浦監控、故障檢測與復歸功能。
 *
 * 【控制目標】
 * - 流量控制模式: 維持系統流量在設定值 (預設 100 L/min)
 * - 壓差控制模式: 維持系統壓差在設定值 (預設 2.5 bar)
 * - 泵浦協調: 根據需求自動調整運行泵浦數量(1-3台)
 *
 * 【感測器配置】
 * - F1/F2/F3/F4: 流量監測 (0.1 L/min 精度)
 * - P15/P16: 泵前壓力 (bar)
 * - P17/P18: 出水壓力 (bar)
 * - 泵浦回饋: 頻率(Hz), 電流(A×0.01), 電壓(V×0.1)
 *
 * 【執行器控制】
 * - AC Pump1/2/3: 速度 0-1000 (0-100%, REG 411037/411039/411041)
 * - 啟停控制: DO (REG 411101/411103/411105)
 * - 異常復歸: DO (REG 411102/411104/411106)
 * - 故障狀態: DI (REG 411109/411110/411111, 0=故障)
 *
 * 【控制模式】
 * - 手動模式: 外部直接設定泵浦速度和啟停,系統僅監控
 * - 自動模式: PID 控制,自動調整泵浦速度和數量
 *
 * 【泵浦協調策略】
 * - 流量控制: 根據PID輸出決定主泵速度,必要時啟動輔泵
 * - 壓差控制: 類似流量控制策略
 * - 主泵輪換: 支援主泵輪換以均勻磨損
 *
 * 【PID 參數】
...
{
    info(tag, "Initializing LX1400T pump controller");
