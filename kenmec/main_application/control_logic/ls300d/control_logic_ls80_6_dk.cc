/*
 * control_logic_ls80_6.c - LS80 比例閥控制邏輯 (Control Logic 6: Proportional Valve Control)
 *
 * 【功能概述】
 * 本模組實現 CDU 系統的比例閥控制功能,通過 PID 演算法控制閥門開度,維持系統流量穩定。
 * 支援手動/自動模式切換,並提供閥門響應監控、故障檢測和緊急保護功能。
 *
 * 【控制目標】
 * - 流量控制模式: 維持 F2 流量在設定值
 * - 預設目標流量: 100.0 L/min
 ...
 *
 * 【感測器配置】
 * - F2流量 (REG 42063): 二次側出水流量 (主要控制目標, 0.1 L/min 精度)
 * - P11壓力 (REG 42092): 二次側進水壓力 (bar, 監控用)
 * - P17壓力 (REG 42098): 二次側出水壓力 (bar, 監控用)
 * - 閥門開度回饋 (REG 411161): 電磁閥實際開度 (4-20mA 對應 0-100%)
 *
 * 【執行器控制】
 * - 閥門命令開度 (REG 411147): 0-100% 開度設定
 * - 手動/自動模式 (REG 45061): 0=自動, 1=手動
 *
 * 【控制模式】
 * - 手動模式: 外部直接設定閥門開度,系統僅監控響應
 * - 自動模式: PID 控制,根據流量自動調整閥門開度
 *
 * 【PID 參數】
 * 流量控制 PID:
 * - Kp: 1.0 (比例增益)
 ...
 * 【閥門控制策略】
 * - 單次最大調整量: 10% (防止劇烈變化)
 * - PID 輸出 → 閥門開度增量 → 限幅 → 寫入命令開度
 * - 開度範圍: 0-100%
 *
 * 【安全保護】
 * - 閥門開度限制: 0-100%
 * - 單次調整量限制: ±10%
 * - 閥門響應容差: ±2% (命令開度 vs 實際開度)
 * - 緊急流量保護:
 *   * 流量過高 (>上限): 緊急減小開度 10%
 *   * 流量過低 (<下限): 緊急增加開度 5%
 *
 ...
 * - 命令開度 vs 實際開度比較
 * - 響應故障檢測 (容差超過 ±2%)
 * - 連續故障 3次 → 切換手動模式 + 設定安全開度 50%
 *
 * 【流量限制】
 * - 流量上限 (REG 45006): 預設 200.0 L/min
 * - 流量下限 (REG 45007): 預設 50.0 L/min
 * - 超出範圍觸發緊急保護
 *
 * 【閥門開度轉換】
 * - 寫入: 百分比 (0-100%) → 寄存器值 (0-100)
 * - 讀取: 寄存器值 (4-20mA) → 百分比 ((value-4)*100/16)
 *
 * 作者: [DK]
 * 日期: 2025
 */
