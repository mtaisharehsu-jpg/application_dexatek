...
/*---------------------------------------------------------------------------
                            Defined Constants
 ---------------------------------------------------------------------------*/
static const char *debug_tag = "ls80_2_pressure";
...
// 壓力監測寄存器
static uint32_t REG_P4_INLET_PRESSURE = 411067;   // P4二次側進水壓力1
//static uint32_t REG_P12_INLET_PRESSURE = 42093;   // P12二次側進水壓力2

// #define REG_P2_OUTLET_PRESSURE    42097   // P17二次側出水壓力1  
// #define REG_P5_WATERBOX_PRESSURE    411163  // P18二次側出水壓力2
static uint32_t REG_P2_OUTLET_PRESSURE = 411065;   // P2側出水壓力1 // [DK]
static uint32_t REG_P5_WATERBOX_PRESSURE = 411163;   // P5二次側水壓力 // [DK]

static uint32_t REG_P1_PRIMARY_INLET = 411061;   // P1一次側進水壓力 (參考)
// #define REG_P3_PRIMARY_OUTLET      42089   // P9一次側出水壓力 (參考)
static uint32_t REG_P3_PRIMARY_OUTLET = 411063;   // P9一次側出水壓力 (參考) // [DK]
...
// 比例閥控制寄存器
static uint32_t REG_VALVE_SETPOINT = 411151;  // 閥門開度設定 (%)
...
#define MIN_PUMP_SPEED             10.0f   // 泵浦最小速度 10%
...
static int _register_list_init(void)
{
    ...
    _control_logic_register_list[2].name = REG_P3_PRESSURE_STR;
    _control_logic_register_list[2].address_ptr = &REG_P3_PRIMARY_OUTLET;
    _control_logic_register_list[2].default_address = REG_P3_PRIMARY_OUTLET;
    _control_logic_register_list[2].type = CONTROL_LOGIC_REGISTER_READ;

    _control_logic_register_list[3].name = REG_P4_PRESSURE_STR;
    _control_logic_register_list[3].address_ptr = &REG_P4_INLET_PRESSURE;
    _control_logic_register_list[3].default_address = REG_P4_INLET_PRESSURE;
    ...
    _control_logic_register_list[5].name = REG_P2_PRESSURE_STR;
    _control_logic_register_list[5].address_ptr = &REG_P2_OUTLET_PRESSURE;
    _control_logic_register_list[5].default_address = REG_P2_OUTLET_PRESSURE;
    _control_logic_register_list[5].type = CONTROL_LOGIC_REGISTER_READ;

    _control_logic_register_list[6].name = REG_P5_PRESSURE_STR;
    _control_logic_register_list[6].address_ptr = &REG_P5_WATERBOX_PRESSURE;
    _control_logic_register_list[6].default_address = REG_P5_WATERBOX_PRESSURE;
...
static int read_pressure_sensor_data(pressure_sensor_data_t *data) {
    ...
    // 讀取P11二次側進水壓力1
    pressure_raw = modbus_read_input_register(REG_P4_INLET_PRESSURE);
    if (pressure_raw >= 0) {
    ...
    // 讀取P17二次側出水壓力1
    pressure_raw = modbus_read_input_register(REG_P2_OUTLET_PRESSURE);
    if (pressure_raw >= 0) {
    ...
    // // 讀取P5二次側出水壓力2
    // pressure_raw = modbus_read_input_register(REG_P5_WATERBOX_PRESSURE);
    // if (pressure_raw >= 0) {
...
static int check_manual_mode(void) {
    ...
    return (pump1_manual > 0 || pump2_manual > 0 || valve_manual > 0);
}
...
static int calculate_pump_coordination_strategy(float pid_output, pump_coordination_strategy_t *strategy) {
    ...
    if (pid_output > 0) {
        ...
        if (pid_output > 50.0f) {
            ...
            // 速度計算: 10% + (PID輸出 - 50) * 1.6
            // 當PID=50%時,每泵速度=10%, 當PID=100%時,每泵速度=90%
            strategy->pump1_speed = MIN_PUMP_SPEED + (pid_output - 50.0f) * 1.6f;
            strategy->pump2_speed = MIN_PUMP_SPEED + (pid_output - 50.0f) * 1.6f;
            info(debug_tag, "雙泵運行策略 - PID輸出: %.1f%%, Pump1速度: %.1f%%, Pump2速度: %.1f%%",
                 pid_output, strategy->pump1_speed, strategy->pump2_speed);

        } else if (pid_output > 30.0f) {
            // 單泵運行 - 低負載
            strategy->pump1_running = 1;
            strategy->pump2_running = 0;
            // 速度計算: 10% + (PID輸出 - 30) * 3.5
            // 當PID=30%時,泵速度=10%, 當PID=50%時,泵速度=80%
            strategy->pump1_speed = MIN_PUMP_SPEED + (pid_output - 30.0f) * 3.5f;
            strategy->pump2_speed = 0.0f;
            info(debug_tag, "單泵運行策略 - PID輸出: %.1f%%, Pump1速度: %.1f%%",
                 pid_output, strategy->pump1_speed);
    ...
    // 限制速度範圍，確保運行中的泵速度 >= MIN_PUMP_SPEED (10%)
    ...
    // 確保運行中的泵速度不低於最小值
    if (strategy->pump1_running && strategy->pump1_speed < MIN_PUMP_SPEED) {
        strategy->pump1_speed = MIN_PUMP_SPEED;
    }
    if (strategy->pump2_running && strategy->pump2_speed < MIN_PUMP_SPEED) {
        strategy->pump2_speed = MIN_PUMP_SPEED;
    }
