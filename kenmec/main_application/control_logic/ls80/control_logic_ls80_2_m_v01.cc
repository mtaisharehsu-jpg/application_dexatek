...
#define CONTROL_DEADZONE          5.0f    // 控制死區 %
...
static int read_pressure_sensor_data(pressure_sensor_data_t *data) {
    ...
    // 計算壓力差 (P4 - P2)
    data->pressure_differential = data->P4_secondary_inlet - data->P2_secondary_outlet;
...
static float calculate_pressure_pid_output(pressure_pid_controller_t *pid, float setpoint, float current_value) {
    ...
    debug(debug_tag, "壓差PID - 誤差: %.2f, P: %.2f, I: %.2f, D: %.2f, 輸出: %.2f%%",
...
static void calculate_pump_control(float pid_output, pump_control_output_t *output) {
    ...
    // 死區處理：小於死區的輸出視為無需調整
    if (fabs(pid_output) < CONTROL_DEADZONE) {
        debug(debug_tag, "PID輸出在死區內，停止所有泵浦");
        return;
    }

    // 簡化策略：兩個泵同速運行
    float pump_speed = fabs(pid_output);

    // 限制速度範圍
    if (pump_speed > PUMP_MAX_SPEED) pump_speed = PUMP_MAX_SPEED;
    if (pump_speed < PUMP_MIN_SPEED && pump_speed > 0) pump_speed = PUMP_MIN_SPEED;
    ...
    debug(debug_tag, "泵浦控制 - Pump1: %.1f%%, Pump2: %.1f%%",
...
int control_logic_ls80_2_pressure_control(ControlLogic *ptr) {
    ...
    if (control_mode == PRESSURE_CONTROL_MODE_AUTO) {
        // 自動模式
        info(debug_tag, "執行自動壓差控制模式 ((P4-P2)→Pset追蹤)");
