# Update Hub Firmware Procedure

## 1. Copy firmware image (.swu) to NFS folder

## 2. Hub mount to NFS folder
...
```bash
cd /mnt/
cp kenmec_OTA_v1.0.1.10.swu /tmp
sysupd
```

## 4. Wait for a while for update finished

```bash
[root@Kenmec:/mnt]# sysupd
Update "/tmp/kenmec_OTA_v1.0.1.10.swu" to Slot B
Swupdate v2020.04.0
