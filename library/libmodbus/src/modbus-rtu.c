...
static modbusRtsCallback _modbus_rts_cb = NULL;

int modbus_transceiver_cb_setup(modbusRtsCallback rtsCallback)
{
    int rc = 0;

    if (rtsCallback) {
        _modbus_rts_cb = rtsCallback;
    }
...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    if (ctx_rtu->rts != MODBUS_RTU_RTS_NONE) {
        ...
        if (_modbus_rts_cb) {
            _modbus_rts_cb(ctx_rtu->rts == MODBUS_RTU_RTS_UP);
        }
